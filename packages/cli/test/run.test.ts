// Part D — `run <issue> --attach` launches an INTERACTIVE claude in the issue workspace
// via the Exec.attach (TTY-inherited) seam. It must NOT touch the headless runner path:
// no `-p`, no `--output-format`, no `stream-json`, and ClaudeRunner is never constructed.
...
describe('attach (D1 — interactive launch in workspace)', () => {
  it('resolves the workspace and attaches claude there, returning the exit code', async () => {
...
describe('attach (D2 — never the headless runner path)', () => {
  it('argv carries no -p / --output-format / stream-json', async () => {
    const exec = new FakeExec();
    await attach(42, { exec, resolveWorkspace: () => '/root/g__r/42' });
...
describe('attach (D3 — missing workspace is a clear error)', () => {
  it('throws a clear error and does NOT spawn when no workspace exists', async () => {
    const exec = new FakeExec();
    await expect(attach(42, { exec, resolveWorkspace: () => undefined })).rejects.toThrow(
      /workspace/i,
    );
