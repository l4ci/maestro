...
export function startDaemon(opts: DaemonOptions = {}): { stop: () => void } {
  ...
  // Resolve the git token from the repo's OWN forge — one daemon serves repos on different
  // forges (gh/glab), each with its own token_env (#15). Shared by the clone path and the
  // WORKFLOW.md fetch.
